...
2. **In the browser**, with [Pyodide](https://pyodide.org/) on the page (and, in `step2b-browser-worker/`, in a Web Worker)
...
| 2 | The browser page (Pyodide) | [`step2-browser/bridge.py`](step2-browser/bridge.py) | a call from the page |
...
- In-process state (the counter) behaves differently per host, which is a feature of the demo: it persists in the Uvicorn process and in the browser page, but on Workers each isolate has its own.
